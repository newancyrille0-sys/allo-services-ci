...
export async function GET(request: Request) {
  ...
  try {
    // Essayer de récupérer les prestataires depuis la base de données
    let dbProviders: any[] = [];
    ...
    try {
      ...
      dbProviders = await db.providerProfile.findMany({
        where: whereClause,
        include: {
          user: {
            select: {
      ...
      // Si erreur DB, utiliser les données mock
      console.log("Using mock providers data");
    }

    // Si pas de données DB, utiliser mock
    let providers = dbProviders.length > 0 
      ? dbProviders.map((p) => {
          ...
          return {
            ...
            service: p.services[0]?.name || "Services",
            city: p.city || "Abidjan",
            rating: Math.round(avgRating * 10) / 10,
            reviewCount: p.reviews.length,
            image: p.user.avatarUrl || "https://via.placeholder.com/100",
            isVerified: p.isVerified,
            isPremium: p.subscriptionTier !== "FREE",
            coordinates: CITIES_COORDINATES[p.city || "Abidjan"] || CITIES_COORDINATES["Abidjan"],
          };
        })
      : MOCK_PROVIDERS.map((p) => ({
          ...p,
          coordinates: CITIES_COORDINATES[p.city] || CITIES_COORDINATES["Abidjan"],
        }));
